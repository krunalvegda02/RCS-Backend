...
//         // Process messages sequentially to avoid overwhelming DB
//         const results = [];
//         for (const message of messages) {
//           if (!isRunning() || isStale()) continue;
...
async function startBatchEntriesConsumer() {
  ...
  try {
    ...
    // Load model dynamically to avoid issues if DB isn't ready at import time, though here we are connected.
    const ContactCampaignMessage = (await import('../models/contact_campaign_message.model.js')).default;
          ...
          try {
            ...
            if (bulkOps.length > 0) {
              try {
                ...
                // console.log(`[BatchConsumer] 💾 BulkWrite result: inserted=${result.insertedCount}, modified=${result.modifiedCount}`);
